...
for table in tables:
    if is_title(table):
        ...
        if (text[25:43].strip() == 'to be arranged'):
            ...
            dates = []
            raw_date_string = text[25:32].strip()
            dates.append(extract_dates(raw_date_string))
            times = []
            times.append(text[32:43].strip())

        building = []
        building.append(text[43:48].strip())

        room = []
        room.append(text[48:57].strip())

        instructor = []
        instructor.append(text[57:84].strip())
        ...
        for line in stripped_lines:
            if has_additional_meeting_times(line):
                raw_date_string = line[0:7].strip()
                dates.append(extract_dates(raw_date_string))
                times.append(line[7:16].strip())
                building.append(line[16:21].strip())
                room.append(line[21:30].strip())
                instructor.append(line[30:57].strip())
